...
async function mount(root,config,compact=false){
  ...
  root.innerHTML='<header class="ac-heading"><div><p class="ac-eyebrow">PERSPECTIVA ANUAL</p><h2>'+(compact?'¿Cómo vamos frente al año anterior?':'El año, en perspectiva.')+'</h2><p>El cierre anual y el mismo periodo, en una sola lectura.</p></div>'+
  ...
  function paint(data,p){
    ...
    content.innerHTML='<div class="ac-kpis">'+card(options.target+' · Periodo analizado',c,range(p.current),'ac-kpi-current')+
      ...
      (!compact?'<div class="ac-chart-header"><div><p class="ac-eyebrow">DOS FORMAS DE LEER EL AÑO</p><h3>Elige tu referencia</h3></div><div class="ac-switch" role="group" aria-label="Referencia de los gráficos"><button type="button" data-mode="same" aria-pressed="'+(options.mode==='same')+'">Mismo rango de fechas</button><button type="button" data-mode="full" aria-pressed="'+(options.mode==='full')+'">Año '+options.base+' completo</button></div></div>':'')+
      ...
      '<div class="ac-panels"><section class="ac-panel"><h3>'+(compact?'Tres cifras para la reunión':'Evolución mensual')+'</h3>'+
      ...
      '<section class="ac-detail"><div class="ac-chart-header"><div><p class="ac-eyebrow">LAS CIFRAS, SIN PERDER CONTEXTO</p><h3>Balance por delito</h3></div><span class="ac-note">— = no disponible / sin base porcentual</span></div>'+table(data,p)+'</section>'+
