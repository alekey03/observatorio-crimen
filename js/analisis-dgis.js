...
window.DgisAnalysis = (() => {
    ...
    async function mount(root, initial, forecast, current=()=>true){
        ...
        function render(){
            ...
            q('#daContent').innerHTML=(findings.length?`<section class="da-reading"><h3>Lectura de la selección</h3><ul>${findings.join('')}</ul><p>Describe las denuncias registradas; no permite concluir por sí solo por qué ocurre el delito.</p></section>`:'')+`<div class="da-grid">${fields.map(i=>panel(i,b)).join('')}</div>`+(tab==='detalle'?`<section class="da-panel"><h3>Qué información falta</h3><p>Denuncias que incluyen algún valor sin determinar. Una denuncia puede tener a la vez un valor informado y otro vacío.</p><div class="da-quality">${descriptions.map(([name],i)=>{const entries=(b?.[4][i]||[]).filter(([v])=>unknown(v));return `<div><strong>${name}</strong><span>${entries.length?entries.map(([v,n])=>`${escape(v)}: ${number(n)}`).join(' · '):total?'Sin categorías vacías detectadas':'Sin registros'}</span></div>`;}).join('')}</div></section>`:'');
