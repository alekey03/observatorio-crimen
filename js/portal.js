...
window.Portal = (() => {
  ...
  const titles = {inicio:['Resumen de denuncias','Una lectura ejecutiva del periodo seleccionado'], 'mapa-delito':['Distribución territorial','Participación e intensidad'], 'mapa-calor':['Distribución territorial','Intensidad geográfica'], 'comparador-delitos':['Comparativo anual','Cierre anual y periodos equivalentes'], 'analisis-temporal':['Análisis temporal de denuncias','Patrones y variaciones observadas'], 'analisis-predictivo':['Proyecciones de denuncias','Estimaciones sujetas a incertidumbre'], 'produccion-policial':['Resultados de la actividad policial','Producción por fecha y dependencia']};


  const journeys = [
    ['mapa-delito','01','fa-map-location-dot','Distribución territorial','Explora departamentos, provincias y distritos.'],
    ['comparador-delitos','02','fa-scale-balanced','Comparación anual de denuncias','Contrasta el mismo periodo y el año completo.'],
    ['analisis-temporal','03','fa-clock','Análisis por fecha de registro','Lee la evolución y los patrones del calendario.'],
    ['analisis-predictivo','04','fa-chart-line',new URLSearchParams(location.search).get('fuente')==='dgis-diaria'?'Características del delito':'Proyecciones de denuncias',new URLSearchParams(location.search).get('fuente')==='dgis-diaria'?'Explora medios, móviles, horarios y calidad del registro.':'Consulta escenarios y márgenes de incertidumbre.'],
    ['produccion-policial','05','fa-chart-column','Producción policial','Revisa los indicadores de producción policial.']
  ];
  function executive({total,territories={},crimes={},source,range,place,crime,territoryLabel='Territorios'}) {
    ...
    return `<section class="ex-home" aria-label="Resumen ejecutivo del periodo"><header class="ex-heading"><div><span class="ex-kicker">RESUMEN DEL PERIODO</span><h2>Resumen territorial y por categoría</h2><p>${esc(range)} · ${esc(place)} · ${esc(source)}</p></div><span class="ex-source"><i class="fas fa-filter" aria-hidden="true"></i> ${esc(crime||'Todos los delitos')}</span></header>
    <div class="ex-lead-grid"><article class="ex-focus"><span class="ex-kicker">TERRITORIO CON MÁS DENUNCIAS</span><h3>${esc(top?.[0]||'Sin registros')}</h3><div class="ex-focus-number">${top?pct(top[1]):'—'}<span>del total seleccionado</span></div><p>${top?fmt(top[1])+' denuncias en este territorio.':'Prueba otro periodo o territorio.'}</p><button type="button" data-ex-view="mapa-delito">Explorar el mapa <span aria-hidden="true">↗</span></button></article>
    <div class="ex-signals"><article><span class="ex-kicker">${crime?'DELITO SELECCIONADO':'CATEGORÍA CON MÁS REGISTROS'}</span><h3>${esc(crime||lead?.[0]||'Sin registros')}</h3><p>${crime?fmt(total):lead?fmt(lead[1]):'0'} denuncias en el periodo.</p><small>Volumen registrado; no es una tasa de riesgo.</small></article><article><span class="ex-kicker">COBERTURA TERRITORIAL</span><strong>${fmt(ts.length)}</strong><p>${esc(territoryLabel.toLowerCase())} con registros</p><small>Según la agrupación territorial publicada.</small></article></div></div>
    <div class="ex-rankings"><section><header><span class="ex-kicker">DISTRIBUCIÓN</span><h3>${crime?'Categoría seleccionada':'Categorías con más denuncias'}</h3></header><ol>${ranking(cs,'crime')}</ol></section><section><header><span class="ex-kicker">TERRITORIO DEL HECHO</span><h3>${esc(territoryLabel)} con más denuncias</h3></header><ol>${ranking(ts,'territory')}</ol></section></div>
    <p class="ex-footnote">Las cifras corresponden a los filtros seleccionados. Una denuncia puede aparecer en varias categorías o territorios, por lo que sus subtotales no deben sumarse. Un lugar con más denuncias no necesariamente es más peligroso: esta comparación no considera cuántas personas viven allí.</p>
    <header class="ex-heading ex-next"><div><span class="ex-kicker">SECCIONES DEL DASHBOARD</span><h3>Acceso a los análisis</h3></div></header><nav class="ex-journeys" aria-label="Explorar el observatorio">${journeys.map(([view,num,icon,title,copy])=>`<button type="button" data-ex-view="${view}"><span class="ex-route"><i class="fas ${icon}" aria-hidden="true"></i><small>${num}</small></span><strong>${esc(title)}</strong><span>${esc(copy)}</span><b aria-hidden="true">↗</b></button>`).join('')}</nav></section>`;
  }
  ...
  async function forecast(source,crime,state) {
    ...
    if(!item || item.evaluation.status!=='ok')return `<section class="portal-empty"><h2>Historial insuficiente para generar proyecciones</h2><p>${esc(item?.evaluation.reason||'No hay una serie evaluada para este delito.')}</p><p>Se requieren al menos 48 meses completos para separar entrenamiento, selección y prueba. No se utiliza SIDPOL en lugar de DGIS.</p></section>`;
