(() => {
    ...
    async function render() {
        ...
        output.querySelectorAll('.selection-total').forEach(el=>el.remove());
        let selectedTotal='';
        try {
            const {result}=await request({type:'query',filters:state});
            if(mine!==revision) return;
            selectedTotal=Portal.selectionTotal(`Total de denuncias · ${state.crime || 'Todos los delitos'}`,result.total,`${state.from||metadata.min_date} al ${state.to||metadata.max_date} · ${[state.department==='@LIMA'?'LIMA (departamento completo)':state.department,state.province,state.district].filter(Boolean).join(' / ')||'Nacional'} · ${config.label}`);
    ...
    function annualConfig(state) {
        return {label:config.label,metadata,state,query:async filters=>(await request({type:'query',filters})).result,open:()=>setView('comparador-delitos')};
    }
    async function comparator(state,mine) {
        if(mine!==revision)return;
        await Annual.mount(document.getElementById('dgisResults'),annualConfig(state));
    }
    function setView(name) {
        ...
        document.getElementById('dgisTitle').textContent=({inicio:'Resumen de denuncias', 'mapa-delito':'Distribución territorial','analisis-temporal':'Análisis temporal de denuncias','comparador-delitos':'Comparativo anual','analisis-predictivo':'Proyecciones de denuncias'})[name] || 'Información no disponible';
